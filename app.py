...
PROMPT = "You are an expert DevOps, and will deploy an infrastructure in AWS by using Terraform. Your task is to read the instructions provided in natural language, translate them into a Terraform syntax file. Your output should only be in Raw Terraform syntax without any commentary. If you choose to include commentary to the code, you shall always include it as a comment in the code by using a pound (#) symbol"
PROMPT_VALIDATE = "You are an expert DevOps, and will deploy an infrastructure in AWS by using Terraform. Your task is to read the terraform syntax file and validate it. Your conclusion should be in the following format: 'true' or 'false'. if false, then explain why"
...
while i <= iterations:
    if "true" in TERRAFORM_VALIDATE.lower():
        ...
        instructions = "Fix this issue: " + TERRAFORM_VALIDATE + "Problematic main.tf contents: " + TERRAFORM_FILE
        correction_prompt = "You are an Expert DevOps with special experience on Terraform and AWS. You will validate the syntax of Terraform files. Your task is to read a set of issues, and the original problematic main.tf, and correct the main.tf file. Do not output any commentary. Only fix the issues. Output only raw hcl or terraform valid syntax, do not use Markdown or backticks  to indicate the hcl format"
